...
public:
    ...
    UwbImuFusion() {
        ...
        ROS_INFO("Gravity in world frame defined as [0, 0, -%.2f] (pointing downward)", gravity_magnitude_);
        ROS_INFO("Using VINS-Mono style IMU pre-integration");
        ROS_INFO("Using frame IDs: %s -> %s for better RViz visualization", world_frame_id_.c_str(), body_frame_id_.c_str());
    }
...
private:
    ...
    std::string world_frame_id_;  // Now "map" by default
    std::string body_frame_id_;   // Now "base_link" by default
    ...
    bool optimizeFactorGraph() {
        ...
        try {
            ...
            // Add roll/pitch prior to all states (for ground vehicles/drones)
            for (size_t i = 0; i < state_window_.size(); ++i) {
            ...
            // Log optimization results with more info
            ROS_INFO("Optimization: %d iterations in %.2f ms. Final cost: %.6f, UWB factors: %d, IMU factors: %d", 
