 ...
 */
 ...
 struct ImuFactor {
     ...
     bool operator()(const T* const state1, const T* const state2, T* residuals) const {
         ...
         // Position residual
         residual.segment<3>(0) = p2 - (p1 + v1 * T(preintegration_.dt_sum_) + 
                               T(0.5) * GRAVITY_VECTOR.cast<T>() * T(preintegration_.dt_sum_ * preintegration_.dt_sum_) +
                               q1 * dp);
 
         // Velocity residual
         residual.segment<3>(3) = v2 - (v1 + GRAVITY_VECTOR.cast<T>() * T(preintegration_.dt_sum_) +
                               q1 * dv);
    ...
    private:
        ...
        void addImuFactor(ceres::Problem& problem, double* state_i, double* state_j) {
            ...
            problem.AddResidualBlock(
                ...
                new ceres::HubbardLossFunction(1.0),
                state_i,
                state_j
            );
        }
