...
public:
    FusionNode() : nh_("~") {
        ...
        imu_sub_ = nh_.subscribe("/imu/data", 1000, &FusionNode::imuCallback, this);
        uwb_sub_ = nh_.subscribe("/vins_estimator/UWBPoistionPS", 10, &FusionNode::uwbCallback, this);
    ...
    void processUWBMeasurement(const geometry_msgs::PointStamped& uwb_msg) {
        ...
        // Add UWB factors (loosely coupled integration)
        Eigen::Vector3d UWBPos(uwb_msg.point.x, uwb_msg.point.xy, uwb_msg.point.z);
        UWBFactorLoose
...
void linearizeResidualBlocks(ceres::Problem* problem, const std::vector<State>& states, Matrix<double, 32, 32>& H, VectorXd& b) {
    // Get all residual blocks connected to oldest state
    std::vector<ceres::ResidualBlockId> old_residuals;
    problem->GetResidualBlocksForParameterBlock(states[0].param_ptr.get(), &old_residuals);

    // Linearize each residual block
    for (auto& residual_id : old_residuals) {
        // Get the cost function and parameter blocks
        const ceres::CostFunction* cf = problem->GetCostFunctionForResidualBlock(residual_id);
        ...
        for (size_t i = 0; i < parameter_blocks.size(); ++i) {
            jacobian_matrices[i].resize(cf->num_residuals(), problem->ParameterBlockSize(parameter_blocks[i]));
